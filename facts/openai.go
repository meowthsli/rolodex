...
1.4 "confidence": "exact" если подтверждено, "probable" если только планирует.
2.1 "properties.exact_quote": Вырежьте точную, дословную цитату из текста, на основе которой вы сделали вывод об этой связи, но без окружающих деталей. Не перефразируйте текст.
2.2 "properties.details": обстоятельства - место, условия, интересный связанный факт, при котором актуальна эта связь, раунд для инвестиций и так далее.
2.3 "properties.amount": сумма сделки, зарплаты, компенсации или любое другое упоминание денег, если известна - в рублях, долларах, в чем нашли; если нет, проставляем "~" (тильду).
2.4 "properties.when": дата при наличии, точная или примерная до года, когда это происходило.

Уникальный ID сущности генерируется из её имени транслитераций и заменой пробелов на underscore; cлова в имени из нескольких слов упорядочить по алфавиту - "Евгений Горин" превращается в "GORIN_EVGENIY" .
...
Структура JSON:
{
  ...
  "relations": [{"source": "ID_субъекта", "target": "ID_объекта", "type": "ТИП_СВЯЗИ", "properties": {"details": "...", "exact_quote": "...", "amount": "...", "when": "..."}}]
}
