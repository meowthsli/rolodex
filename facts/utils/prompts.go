...
Для каждой связи (claim) вы ОБЯЗАНЫ заполнить атрибуты:
...
Структура JSON:
{
  ...
  "claims": [{"source": "ID_субъекта", "target": "ID_объекта", "type": "ТИП_СВЯЗИ", "properties": {"details": "...", "exact_quote": "...", "amount": "...", "when": "...", "conf": "..."}}]
}
...
1.3 "type": тип связи - из известных типов.
2.1 "props.exact_quote": Вырежьте точную, дословную цитату из текста, на основе которой вы сделали вывод об этой связи, но без окружающих деталей. Не перефразируйте текст.
2.2 "props.details": обстоятельства места: страна, город, другой интересный факт, привязывающий событие.
2.3 "props.amount": сумма сделки, зарплаты, компенсации или любое другое упоминание денег, если известна; если нет, проставляем "~" (тильду).
2.4 "props.when": дата при наличии, точная или примерная до года, когда это происходило.
2.5 "props.conf": "exact" если подтверждено, "probable" если только планирует.
